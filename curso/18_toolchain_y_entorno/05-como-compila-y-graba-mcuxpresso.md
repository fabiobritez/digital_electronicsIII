...
`.ld`, que es lo que hacemos en el [módulo 16](../16_build_linker_startup/) y en el `mygpio` de este
repo.
...
direcciones hardcodeadas, las lee de la tabla. Es más elegante que el startup mínimo del módulo 16,
...
├── binaries/
│   ├── redlinkserv               el servidor que habla con la sonda por USB
...
```
Eclipse  ->  arm-none-eabi-gdb  ->  crt_emu_cm_redlink  ->  redlinkserv  ->  USB  ->  sonda  ->  SWD  ->  LPC1769
```
...
1. La sonda halla el core y **copia el driver `.cfx` a la RAM** del LPC1769.
...
| genera 3 linker scripts desde plantillas | escribís **un** `.ld` a mano (módulo 16) |
...
**Ver también:** [16 - Build, linker y startup](../16_build_linker_startup/)
