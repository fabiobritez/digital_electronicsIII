...
En [`setup/.vscode/`](./setup/.vscode/) de este módulo están listos para copiar. Qué hace cada uno:
...
Define una **tarea de build** que corre tu compilación (el `build.sh` o el `make` del módulo 16):
...
```jsonc
{
  "tasks": [{
    ...
    "type": "shell",
    "command": "./build.sh",
    "options": { "cwd": "${workspaceFolder}/curso/02_arma_tu_propia_libreria/src/build" },
...
[`setup/.vscode/launch.json`](./setup/.vscode/launch.json).
...
│   └── main.c
├── startup.c                <- módulo 16
├── lpc1769.ld               <- módulo 16
├── build.sh  (o Makefile)   <- módulo 16
├── library/  (CMSIS)        <- los drivers
└── tools/toolchain/         <- el compilador (gitignored)
```

(En este repo, la plantilla de `02_arma_tu_propia_libreria/src/build/` ya tiene `build.sh`, `startup.c`
y `lpc1769.ld` funcionando.)
