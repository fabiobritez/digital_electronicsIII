...
### El vocabulario propio de NXP: `ENABLE`, `SUCCESS`, `SET`

Apenas empieces a usar los drivers te vas a cruzar con esto:

```c
CLKPWR_ConfigPPWR(CLKPWR_PCONP_PCTIM0, ENABLE);   // módulo 3
match.IntOnMatch = ENABLE;                        // módulo 8
if (I2C_MasterTransferData(...) != SUCCESS) { }   // módulo 13
```

`ENABLE` y `SUCCESS` **no son de C**. Son constantes de unos `enum` que define NXP en
[`Drivers/inc/lpc_types.h`](../../library/CMSISv2p00_LPC17xx/Drivers/inc/lpc_types.h), y conviene
tenerlas identificadas porque aparecen en toda la capa 3:
...
| Tipo | Valores | Para qué lo usan los drivers |
|---|---|---|
| `FunctionalState` | `ENABLE` / `DISABLE` | prender o apagar algo: `TIM_Cmd(LPC_TIM0, ENABLE)` |
| `Status` | `SUCCESS` / `ERROR` | resultado de una operación que puede fallar |
| `FlagStatus`, `IntStatus`, `SetState` | `SET` / `RESET` | estado de una bandera o de un bit |
| `Bool` | `TRUE` / `FALSE` | booleano, previo a `<stdbool.h>` |

Tres cosas para no confundirse:

- **Son cuatro nombres para lo mismo: 0 y 1.** El tipo distinto es solo documentación. Nada te
  impide escribir `TIM_Cmd(LPC_TIM0, 1)`, pero `ENABLE` se lee mejor y es lo que espera la firma.
- **Cuidado con el orden en `Status`:** `SUCCESS` vale **1** y `ERROR` vale **0**, al revés de la
  convención de Unix donde 0 es éxito. Por eso se compara `!= SUCCESS`, nunca `!= 0`.
- **`FlagStatus`, `IntStatus` y `SetState` son el mismo `enum`** declarado con tres nombres. Verlos
  distintos en la documentación no significa que sean tipos distintos.

> [!WARNING]
> El mismo `lpc_types.h` define también una familia vieja: `UNS_32`, `INT_32`, `UNS_16`, `CHAR`,
...
`enum`, y va a estar igual de bien.
